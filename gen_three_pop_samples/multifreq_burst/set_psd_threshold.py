...
def build_argument_parser() -> ArgumentParser:
    ...
    parser.add_argument("--std_ratio", default=3.29)
    return parser


def load_rank_info():
    with open("./rank_info.txt", "r") as fp:
        # NOTE: hard fix file name, need to be corrected in the future
        nrank_set = [int(n) for n in fp.readline().split(",")[:-1]]
    return np.array(nrank_set, dtype=int)


def load_cluster_info(fdir_data):
    cinfo = {"cluster_id": [], "cluster_order": []}
...
def get_th_abs(psd_m, psd_m2):
    N = psd_m.shape[0]
    th_m = np.zeros([N, 2])
    th_s = np.zeros([N, 2])

    nrank_set = load_rank_info()
...
def main(std_ratio=0, fdir_data=None, fout=None, method=None):
    ...
    # get threshold
    if method == "absolute":
        th_m, th_s = get_th_abs(psd_m, psd_m2)
