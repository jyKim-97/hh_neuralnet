...
def main(fdir=None, nc=0, wset=None, tbin=1, seed=42):
    np.random.seed(seed)
    ...
    prob_spk_set, prob_spk_set_p, t = collect_response(summary_obj, winfo, nc, wset, tbin)
    
    # build dataset
    # wlabels = [od.get_motif_labels()[w] for w in wset]
    # wlabels = tuple(wlabels)
    # print(wlabels)
    ds = xr.Dataset(
            data_vars=dict(
            prob=(("nw", "ndelay", "ntp", "t"), prob_spk_set),
            prob0=(("nw", "ndelay", "ntp", "t"), prob_spk_set_p)
        ),
        coords=dict(
            nw=wset,
    ...
    fout = os.path.join(fdir_post, "prob_spk_set_nc%d.nc"%(nc))
    # print(fout)
    ds.to_netcdf(fout)
...
def collect_response(summary_obj, winfo, nc, nw_set, tbin):
    ...
    num_spk_set  = np.zeros((len(nw_set), num_delay, 2, len(nspk_edges)-1))
    num_spk_set_p = np.zeros((len(nw_set), num_delay, 2, len(nspk_edges)-1))
    max_spk = np.zeros((len(nw_set), num_delay, 2)) # ntp (Transmitter; F, S)
    ...
    for nd in tqdm(range(num_delay)):
        for nt in range(summary_obj.num_controls[2]):        
            ...
            for idt, idr in zip(sel_tr[0], sel_tr[1]):
                n0 = 0
                ...
                for nstep in detail["step_spk"][idt]:
                    ...
                    num_spk_set[wid, nd, ntp] += num_spk_r
                    ...
                    num_spk_set_p[wid, nd, ntp] += num_spk_r
                    
                    max_spk[wid, nd, ntp] += 1
            
    prob_spk_set   = num_spk_set / max_spk[:,:,:,np.newaxis]
    prob_spk_set_p = num_spk_set_p / max_spk[:,:,:,np.newaxis]
    # prob_spk_set   = num_spk_set 
    # prob_spk_set_p = num_spk_set_p 
