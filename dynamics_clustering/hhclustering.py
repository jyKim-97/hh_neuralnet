...
class SLHC:
    ...
    def __init__(self, metric="precomputed", method="complete"):
        ...
        self.model = AgglomerativeClustering(n_clusters=None, affinity=metric, linkage=method, distance_threshold=0)
...
def draw_with_dendrogram(res_linkage, cmat, cluster_id, label=None, color_threshold=None, fig=None):
    from scipy.cluster.hierarchy import dendrogram
    ...
    if color_threshold is None:
        color_threshold = max(res_linkage[:, 2])
    dendrogram(res_linkage, no_labels=True, color_threshold=color_threshold)
...
def show_cluster_summary(cluster_id, wsets=None, max_cid=15, min_cid=1, sval=None, sth=0.2,
    ...
    """
    Show square cluster summary
    
    Args
    ----
    cluster_id : iterable (i, j, k, l)
        cluster ID. (i, j, k, l) = (alpha, beta, echelon, w)
    wsets : iterable
        exact value of asymmetry index
    max_cid : int
    min_cid : int
    sval (optional) : iterable (i, j, k, l) (optional)
    sth (optional) : float (0-1)
    figsize (optional) : iterable
    """
    
    num_w = cluster_id.shape[-1]
    num_r = cluster_id.shape[-2]
