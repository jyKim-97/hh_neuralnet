...
def export_sample_figs(cluster_id, silhouette_vals, col_names, nshow=3):
    k0 = min(cluster_id)
    k1 = max(cluster_id)
    targets = []
    for target_cid in range(k0, k1+1):
        ...
        for case in ("best", "intermediate", "worst"):
            tags = hc.show_sample_cases(target_cid, cluster_id, silhouette_vals, col_names, case=case, nshow=nshow, save=True)
...
def main():
    ...
    export_sample_figs(rcluster_id, rsval, col_names, nshow=3)
